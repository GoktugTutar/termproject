...
export class SystemFeedbackService {
  ...
  async getMessages(userId: number) {
    ...
    console.log(
      `[SF] userId=${userId} now=${now.toISOString().substring(0, 10)}`,
    );
    console.log(
      `[SF] feedbacks=${recentFeedbacks.length} lessons=${lessons.length} checklists=${recentChecklists.length} thisWeekBlocks=${scheduledThisWeek.length}`,
    );
    if (profile)
      console.log(
        `[SF] completion7d=${(profile.completionRate7d * 100).toFixed(0)}% stress7d=${profile.avgStress7d.toFixed(1)}`,
      );
        ...
        if (lessonCompletion < 0.55) {
          if (
            !(await this.isOnCooldown(
              userId,
              'sure_isteme_ama_tamamlayamama',
              lf.lessonId,
            ))
          ) {
            console.log(
              `[SF] ✓ sure_isteme_ama_tamamlayamama -> ${lesson.name} (completion=%${Math.round(lessonCompletion * 100)})`,
            );
            messages.push({
            ...
            console.log(
              `[SF] ⏭ sure_isteme_ama_tamamlayamama -> ${lesson.name} (cooldown)`,
            );
          }
        }
      }
    ...
    for (const block of scheduledThisWeek) {
      blocksPerLesson[block.lessonId] =
        (blocksPerLesson[block.lessonId] ?? 0) + block.blockCount;
    }
    ...
    for (const lesson of lessons) {
      ...
      if (lessonPlanned >= 2 && lessonCompleted / lessonPlanned < 0.6) {
        if (!(await this.isOnCooldown(userId, 'sinav_az_calisma', lesson.id))) {
          console.log(
            `[SF] ✓ sinav_az_calisma -> ${lesson.name} (${daysLeft}g kaldi, %${Math.round((lessonCompleted / lessonPlanned) * 100)} tamamlandi)`,
          );
          messages.push({
      ...
      if (allocatedBlocks < 4) {
        if (!(await this.isOnCooldown(userId, 'sinav_az_blok', lesson.id))) {
          console.log(
            `[SF] ✓ sinav_az_blok -> ${lesson.name} (${daysLeft}g kaldi, ${allocatedBlocks} blok)`,
          );
          messages.push({
            type: 'sinav_az_blok',
      ...
      if (last2.every((f) => f.weekloadFeedback === 'cok_yogundu')) {
        if (!(await this.isOnCooldown(userId, 'asiri_yuk'))) {
          ...
          messages.push({
            type: 'asiri_yuk',
            message:
              'Your schedule has felt too heavy for two weeks in a row. Next week has been automatically lightened by 15%.',
    ...
    for (const lesson of lessons) {
      if (lesson.keyfiDelayCount >= 2) {
        if (!(await this.isOnCooldown(userId, 'sik_erteleme', lesson.id))) {
          console.log(
            `[SF] ✓ sik_erteleme -> ${lesson.name} (${lesson.keyfiDelayCount}x)`,
          );
          messages.push({
            type: 'sik_erteleme',
    ...
    if (
      last3Checklists.length >= 3 &&
      last3Checklists.every((c) => c.stressLevel >= 4)
    ) {
      if (!(await this.isOnCooldown(userId, 'yuksek_stres'))) {
        console.log(
          `[SF] ✓ yuksek_stres (levels: ${last3Checklists.map((c) => c.stressLevel).join(',')})`,
        );
        messages.push({
          type: 'yuksek_stres',
          message: 'Your stress level has been high (≥4) for 3 days in a row.',
          suggestion:
            'Consider reviewing your workload or planning a lighter week.',
        });
    ...
    if (
      ...
      if (!(await this.isOnCooldown(userId, 'hareketsizlik'))) {
        ...
        messages.push({
          ...
          suggestion:
            'Try starting with a short session today — even 30 minutes makes a difference.',
        });
    ...
    for (const lesson of lessons) {
      for (const exam of lesson.exams) {
        const daysLeft = Math.ceil(
          (new Date(exam.examDate).getTime() - now.getTime()) /
            (1000 * 60 * 60 * 24),
        );
    ...
    if (upcomingExams.length > 0) {
      console.log(
        `[SF] sinav_yogunlugu: ${upcomingExams.length} sinav score=${examIntensityScore.toFixed(1)}`,
      );
    }

    if (examIntensityScore >= 9) {
      const examList = upcomingExams
        .map((e) => `${e.lessonName} (${e.daysLeft}d)`)
        .join(', ');
      if (!(await this.isOnCooldown(userId, 'sinav_yogunlugu_kritik'))) {
        ...
        messages.push({
          ...
          suggestion:
            'Your exam lessons are the priority right now. Everything else can wait.',
        });
      ...
      if (!(await this.isOnCooldown(userId, 'sinav_yogunlugu_yogun'))) {
        ...
        messages.push({
          ...
          suggestion:
            'Give extra attention to your exam lessons this week — other topics can stay light.',
        });
      ...
      if (!(await this.isOnCooldown(userId, 'sinav_yogunlugu_orta'))) {
        ...
        messages.push({
          ...
          suggestion:
            'A bit of extra focus on your exam lessons now will make a real difference later.',
        });
    ...
    const nextWeekTotalBlocks = nextWeekBlocks.reduce(
      (sum, b) => sum + b.blockCount,
      0,
    );

    if (nextWeekTotalBlocks > 0 && profile) {
      const thisWeekTotal = scheduledThisWeek.reduce(
        (sum, b) => sum + b.blockCount,
        0,
      );
      const avgCapacity =
        profile.completionRate7d * (thisWeekTotal || nextWeekTotalBlocks);
      const capacityRatio =
        avgCapacity > 0 ? nextWeekTotalBlocks / avgCapacity : 1;

      console.log(
        `[SF] program_yogunlugu: nextWeek=${nextWeekTotalBlocks} avgCapacity=${avgCapacity.toFixed(1)} ratio=${capacityRatio.toFixed(2)}`,
      );

      if (capacityRatio > 1.25) {
        if (!(await this.isOnCooldown(userId, 'program_yogunlugu_asim'))) {
          console.log(
            `[SF] ✓ program_yogunlugu_asim (%${Math.round((capacityRatio - 1) * 100)} fazla)`,
          );
          messages.push({
            ...
            suggestion:
              'This is a heads-up, not a change. If the week feels too heavy, your feedback after it will help the system adjust.',
          });
        ...
        if (!(await this.isOnCooldown(userId, 'program_yogunlugu_dusuk'))) {
          ...
          messages.push({
            ...
            message:
              "Next week looks lighter than usual — a good window to catch up on anything you've fallen behind on.",
            suggestion:
              'Light weeks are valuable. Use the extra headroom to consolidate rather than coast.',
          });
    ...
    const hasYuksekDelay =
      messages.filter((m) => m.type === 'sik_erteleme').length >= 2;
    ...
    if (hasYogunHafta && hasYuksekStres) {
      if (!(await this.isOnCooldown(userId, 'kombine_yogun_stres'))) {
        ...
        messages.push({
          ...
          message:
            'Your schedule is heavy and your stress is high at the same time. This combination can seriously hurt your learning.',
          suggestion:
            "Shorten today's study blocks — rescheduling is critical.",
        });
    ...
    if (hasYogunHafta && hasYuksekDelay) {
      if (!(await this.isOnCooldown(userId, 'kombine_yogun_erteleme'))) {
        ...
        messages.push({
          ...
          message:
            'A heavy week is ahead and your delay rate has historically increased under this kind of load.',
          suggestion:
            'The planner is already aware of this pattern — it has prioritised your delayed lessons. Try to honour at least one session for each.',
        });
    ...
    if (recentSleep.length >= 2) {
      ...
      if (badSleepDays.length >= 2 && avgStress >= 3.5) {
        if (!(await this.isOnCooldown(userId, 'dusuk_uyku_stres'))) {
          ...
          let message =
            'Your sleep has been irregular lately and your stress is running high.';
          let suggestion =
            "Try to stabilise your sleep first — it's the foundation of study performance.";
          ...
          console.log(
            `[SF] ✓ dusuk_uyku_stres (badSleep=${badSleepDays.length}/3 stress=${avgStress.toFixed(1)} goodComp=${goodComp?.toFixed(2) ?? '-'} badComp=${badComp?.toFixed(2) ?? '-'})`,
          );
    ...
    console.log(
      `[SF] SONUC: ${messages.length} mesaj → [${messages.map((m) => m.type).join(', ')}]`,
    );

    const aiMessage = await this.buildAiMessage(
      messages,
      profile,
      lastFeedback?.weekloadFeedback ?? null,
    );
    const examResultMessage = this.buildExamResultReminderMessage(
      examResultReminderLessons,
    );
  ...
  private async writeReviewBlockOverride(
    ...
    console.log(
      `[SF] override yazildi: lessonId=${lessonId} addReviewBlock=true`,
    );
  }
  ...
  private async writePriorityOverride(
    ...
    console.log(
      `[SF] override yazildi: lessonId=${lessonId} priorityBoost=+${priorityBoost}`,
    );
  }
  ...
  private readonly cooldownHours: Record<string, number> = {
    ...
    sinav_sonucu_gir: 24, // 1 day  — only relevant immediately after the exam
  };
  ...
  private buildExamResultReminderMessage(lessonNames: string[]): string {
    ...
    const extra =
      uniqueNames.length > 2 ? ` ve ${uniqueNames.length - 2} ders daha` : '';
  ...
  private daysUntilExam(
    ...
    const future = lesson.exams
      .map((e) =>
        Math.ceil(
          (new Date(e.examDate).getTime() - now.getTime()) /
            (1000 * 60 * 60 * 24),
        ),
      )
      .filter((d) => d >= 0);
